...
import type { PlaygroundGeneration } from "@/lib/playground/history";
    ...
    <div className="grid gap-4 lg:grid-cols-[320px_1fr]">
      ...
      >
          ...
          <div className="mt-2 space-y-2">
            {IMAGE_MODELS.map((option) => (
              ...
              >
                ...
                <span className="mt-1 block text-xs text-muted">{option.note}</span>
              </button>
            ))}
          </div>
        </fieldset>
        ...
        <p className="text-center text-xs text-muted">
          {configured ? `${model} · admin only` : "XAI_API_KEY is not configured here"}
        </p>
      ...
      <div className="space-y-4">
        ...
        {generations.length === 0 ? (
          <div className="rounded-2xl border border-border p-8">
            <h2 className="text-xl font-semibold">Nothing generated yet</h2>
            <p className="mt-3 text-muted">
              Write a prompt and generate. Results are stored privately and listed here newest
              first.
            </p>
          </div>
        ) : (
          generations.map((generation) => (
            ...
            >
              ...
              {generation.images.length === 0 ? (
                <p className="text-sm text-muted">
                  These images are stored but their links have expired. Reload to sign new ones.
                </p>
              ) : null}
            </article>
          ))
