...
export default async function PredictionLedgerPage({ searchParams }: { searchParams: Promise<{ kind?: string; page?: string; entity?: string }> }) {
  ...
  return <div className="space-y-6">
    <div><p className="text-sm uppercase tracking-widest text-accent">Creator destination</p><h1 className="mt-3 text-3xl font-semibold">Prediction Ledger</h1>
      <p className="mt-3 text-muted">Creator plays, bets, packages and grades from The Prediction Ledger. This view is private to your approved admin account.</p></div>
    ...
    {error ? <p role="alert" className="rounded-xl border border-red-400/30 p-5 text-red-300">The Creator feed could not be loaded. Refresh to try again.</p>
      : !data?.length ? <div className="rounded-2xl border border-border p-8"><h2 className="text-xl font-semibold">No {kind === "pick" ? "plays" : kind === "bet" ? "bets" : "packages"} received yet</h2><p className="mt-3 text-muted">Connect M2MEC in Creator, then select it when publishing. Delivered records and later status changes will appear here.</p></div>
      : <div className="grid gap-4">{data.slice(0, 30).map((row) => {
        const record = object(row.record);
        const creator = object(record.creator);
        const bet = object(record.bet);
        const money = (value: unknown) => value == null ? "Not supplied" : new Intl.NumberFormat("en-US", { style: "currency", currency: /^[A-Z]{3}$/.test(text(bet.currency)) ? text(bet.currency) : "USD" }).format(Number(value));
        ...
        return <article id={row.entity_id} key={row.entity_id} className={`rounded-2xl border border-border p-6 ${retired ? "opacity-70" : "bg-surface"}`}>
          ...
          {kind !== "package" ? <><p className="mt-2 text-sm">{String(record.units)}U · {record.oddsAmerican == null ? "Price unavailable" : `${Number(record.oddsAmerican) > 0 ? "+" : ""}${String(record.oddsAmerican)}`} · {text(record.pickType)}</p>
            <ol className="mt-4 grid gap-2">{legs.map((leg, index) => <li key={index} className="rounded-lg border border-border p-3"><p>{partnerLegLabel(leg)} · {text(leg.marketType).replaceAll("_", " ")}</p><p className="mt-1 text-sm text-muted">{text(leg.eventName)} · {time(leg.startsAt)}</p>{kind === "bet" ? <p className="mt-1 text-xs text-muted">{leg.acceptedOddsAmerican == null ? "Leg odds not supplied" : `Leg odds: ${Number(leg.acceptedOddsAmerican) > 0 ? "+" : ""}${String(leg.acceptedOddsAmerican)}`} · {leg.isLive === true ? "Live" : leg.isLive === false ? "Pre-game" : "Timing not supplied"}</p> : null}{leg.grade ? <p className="mt-1 text-xs text-muted">Leg: {text(leg.grade)}</p> : null}</li>)}</ol>
          ...
          {kind === "bet" ? <div className="mt-4 space-y-2 text-sm"><p>Bet · {text(bet.sourceMethod).replaceAll("_", " ")} · {text(bet.verificationStatus).replaceAll("_", " ")}</p><p>{text(bet.sportsbook)} · Cash stake {money(bet.cashStake)} · Bonus stake {money(bet.bonusStake)}</p><p>Potential return {money(bet.potentialReturn)} · Settled return {money(bet.settledReturn)} · Settled net {money(bet.settledNet)}</p><p>Placed {time(bet.placedAt)} · Sportsbook status: {text(bet.providerStatus)}</p><p className="text-muted">Ledger grading and sportsbook settlement are tracked separately. Manual test entries are not sportsbook verified.</p></div> : null}
          ...
          {kind !== "bet" && record.analysis ? <p className="mt-4 whitespace-pre-wrap text-sm leading-6">{text(record.analysis)}</p> : null}
